...
<body class="bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-50">
    <?php if ($user->isLoggedIn()): ?>
        ...
        $messageCount = 0;
        try {
            $notifResult = $database->fetchOne("SELECT COUNT(*) as cnt FROM notifications WHERE status = 'Sent'");
            $notifCount = $notifResult['cnt'] ?? 0;
        ...
        <div class="fixed top-0 left-0 md:left-64 right-0 bg-white dark:bg-slate-900 border-b border-slate-200/50 dark:border-slate-700/50 h-16 flex items-center justify-between px-8 z-40">
            ...
            <div class="flex items-center gap-4">
                ...
                <a href="<?php echo $basePath; ?>/pages/notifications.php" class="relative inline-flex items-center text-slate-600 dark:text-slate-400 hover:text-blue-900 transition-colors">
