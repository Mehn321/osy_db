        ...
        <form method="POST" id="profile-form" class="p-8 space-y-6" style="display: none;">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="occupation" class="w-full bg-slate-100 dark:bg-slate-700 border-none rounded-xl py-3 px-4 text-sm focus:ring-2 focus:ring-blue-900 text-slate-900 dark:text-white" placeholder="e.g., Farmer, Student, Vendor">
                </div>
            </div>
            ...
            $selectedBarangay = ($_SESSION['role'] === 'sk_chairman') ? ($_SESSION['barangay'] ?? '') : '';
            $selectedPurok = '';
            $lockBarangay = ($_SESSION['role'] === 'sk_chairman') ? ($_SESSION['barangay'] ?? '') : null;
