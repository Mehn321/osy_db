...
<div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
    <div class="space-y-1">
        ...
        <p class="text-slate-600 max-w-2xl">Manage vocational training programs and scholarships available for youth upskilling.</p>
    </div>
...
<div class="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm mb-8">
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-4 items-end">
        <div>
            ...
            <input type="text" id="train_search" oninput="filterTraining()" placeholder="Search opportunities..." class="w-full bg-slate-100 dark:bg-slate-700 rounded-xl py-3 px-4 text-sm text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-900 border border-transparent transition-all" />
        </div>
        <div>
            <label class="text-xs font-bold text-slate-600 dark:text-slate-400 uppercase pl-1">Type</label>
            <select id="train_type" onchange="filterTraining()" class="w-full bg-slate-100 dark:bg-slate-700 rounded-xl py-3 px-4 text-sm text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-900 border border-transparent transition-all">
        ...
        <div>
            ...
            <select id="train_status" onchange="filterTraining()" class="w-full bg-slate-100 dark:bg-slate-700 rounded-xl py-3 px-4 text-sm text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-900 border border-transparent transition-all">
        ...
        <div class="flex gap-3">
            <a href="training-programs.php" class="w-full py-3 bg-slate-100 dark:bg-slate-700 text-slate-900 dark:text-white rounded-xl font-bold text-sm text-center hover:bg-slate-200 dark:hover:bg-slate-600 transition-all">Reset List</a>
        </div>
    </div>
</div>
    ...
    <?php foreach ($opportunities as $opp): ?>
        <div class="train-card bg-white dark:bg-slate-800 rounded-xl overflow-hidden shadow-sm border border-slate-200 dark:border-slate-700 hover:shadow-lg transition-all"
            ...
            <div class="px-6 py-4 bg-slate-100 dark:bg-slate-700 flex flex-wrap gap-2">
                <button onclick="viewOpportunityDetail(<?php echo $opp['id']; ?>)" class="flex-1 min-w-[120px] py-2 px-3 bg-blue-900 text-white rounded-lg text-sm font-semibold hover:bg-blue-800 transition-colors flex items-center justify-center gap-2">
                    <span class="material-symbols-outlined text-base">visibility</span>
                    View Details
                </button>
                <?php if ($opp['type'] === 'Vocational Training'): ?>
                    <button onclick="openBroadcastModal(<?php echo htmlspecialchars(json_encode($opp)); ?>)" class="py-2 px-3 bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400 hover:bg-purple-200 rounded-lg transition-colors flex items-center gap-1 font-bold text-xs">
    ...
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="bg-white dark:bg-slate-800 rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <form id="opportunityForm" method="POST" class="p-6 space-y-4">
                ...
                <div>
                    <label class="text-sm font-bold text-slate-700 dark:text-slate-300 mb-2 block">Training Provider / Sponsor</label>
                    <input type="text" name="training_provider" id="opp_training_provider" placeholder="e.g., TESDA, LGU Mayor's Office, DOLE" class="w-full bg-slate-100 dark:bg-slate-700 border-none rounded-xl py-3 px-4 text-sm focus:ring-2 focus:ring-blue-900 text-slate-900 dark:text-white">
                </div>

                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="text-sm font-bold text-slate-700 dark:text-slate-300 mb-2 block">Duration</label>
                        <input type="text" name="duration" id="opp_duration" placeholder="e.g., 6 Months, 4 Weeks" class="w-full bg-slate-100 dark:bg-slate-700 border-none rounded-xl py-3 px-4 text-sm focus:ring-2 focus:ring-blue-900 text-slate-900 dark:text-white">
                    </div>
                    <div>
                        <label class="text-sm font-bold text-slate-700 dark:text-slate-300 mb-2 block">Modality</label>
                        <select name="modality" id="opp_modality" class="w-full bg-slate-100 dark:bg-slate-700 border-none rounded-xl py-3 px-4 text-sm focus:ring-2 focus:ring-blue-900 text-slate-900 dark:text-white">
                ...
                <div>
                    <label class="text-sm font-bold text-slate-700 dark:text-slate-300 mb-2 block">Certification Granted</label>
                ...
                <div>
                    <label class="text-sm font-bold text-slate-700 dark:text-slate-300 mb-2 block">Program Description</label>
                    <textarea name="description" id="opp_description" rows="3" placeholder="Details about the training program..." class="w-full bg-slate-100 dark:bg-slate-700 border-none rounded-xl py-3 px-4 text-sm focus:ring-2 focus:ring-blue-900 text-slate-900 dark:text-white"></textarea>
                </div>
    ...
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="bg-white dark:bg-slate-800 rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <form method="POST" class="p-6 space-y-6">
                ...
                <div class="space-y-4">
                    <div>
                        <label class="text-sm font-bold text-slate-700 dark:text-slate-300 mb-2 block tracking-wide uppercase">Message Template (Customizable)</label>
                        <textarea name="custom_message" id="broadcastMessageArea" rows="6" required class="w-full bg-slate-100 dark:bg-slate-700 border-none rounded-xl py-3 px-4 text-sm focus:ring-2 focus:ring-purple-600 text-slate-900 dark:text-white font-mono" onkeyup="updateBroadcastPreview()"></textarea>
                        <p class="text-[10px] text-slate-500 mt-2">Available Variables: <span class="font-bold">{{name}}</span>, <span class="font-bold">{{opportunity}}</span></p>
                    </div>

                    <div class="bg-purple-50 dark:bg-purple-900/20 p-4 rounded-xl border border-purple-100 dark:border-purple-800">
                        <p class="text-xs font-bold text-purple-800 dark:text-purple-400 mb-2 uppercase tracking-wide">Live Preview for <span id="previewRecipient">User</span></p>
                        <div id="broadcastPreviewBox" class="text-sm text-slate-700 dark:text-slate-300 whitespace-pre-wrap italic"></div>
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="text-sm font-bold text-slate-700 dark:text-slate-300 mb-2 block">Recipient Group</label>
                            <select name="target_group" id="broadcast_target_group" required class="w-full bg-slate-100 dark:bg-slate-700 border-none rounded-xl py-3 px-4 text-sm focus:ring-2 focus:ring-purple-600 text-slate-900 dark:text-white" onchange="toggleSpecificBroadcastRecipients()">
                    ...
                    <div id="broadcast_specific_container" class="hidden">
                        <label class="text-sm font-bold text-slate-700 dark:text-slate-300 mb-2 block">Select Specific Recipients</label>
                        <input type="text" id="bc_recip_search" placeholder="Search recipients..." class="w-full mb-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg py-2 px-3 text-xs" onkeyup="filterBCRecipients()">
                        <div class="w-full bg-slate-100 dark:bg-slate-700 rounded-xl p-4 h-48 overflow-y-auto border border-slate-200 dark:border-slate-600 grid grid-cols-1 sm:grid-cols-2 gap-2" id="bc_recip_list">
                            <?php foreach ($allProfiles as $p): ?>
                                <label class="bc-recip-item flex items-center gap-2 cursor-pointer p-2 hover:bg-slate-200 dark:hover:bg-slate-600 rounded-lg transition-colors" data-name="<?php echo strtolower($p['first_name'] . ' ' . $p['last_name']); ?>">
                                    <input type="checkbox" name="specific_ids[]" value="<?php echo $p['id']; ?>" class="w-4 h-4 text-purple-600 border-slate-300 rounded focus:ring-purple-600">
                                    <div class="flex flex-col">
                                        <span class="text-xs text-slate-800 dark:text-slate-200 font-bold"><?php echo htmlspecialchars($p['first_name'] . ' ' . $p['last_name']); ?></span>
                                        <span class="text-[10px] text-slate-500 uppercase"><?php echo htmlspecialchars($p['profile_type']); ?></span>
                                    </div>
                                </label>
                            <?php endforeach; ?>
                        </div>
                    ...
                    <div class="pt-4 border-t border-slate-200 dark:border-slate-700">
                        <h4 class="text-sm font-bold text-slate-700 dark:text-slate-300 mb-3 uppercase tracking-tighter">Delivery Hooks</h4>
                        <div class="flex flex-wrap gap-6">
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="checkbox" name="send_sms" class="w-4 h-4 text-purple-600 bg-slate-100 border-slate-300 rounded focus:ring-purple-600" checked>
                                <span class="text-sm text-slate-600 dark:text-slate-300 font-semibold">Traccar Cloud SMS</span>
                            </label>
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="checkbox" name="send_email" class="w-4 h-4 text-purple-600 bg-slate-100 border-slate-300 rounded focus:ring-purple-600" checked>
                                <span class="text-sm text-slate-600 dark:text-slate-300 font-semibold">Gmail SMTP Email</span>
                            </label>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex justify-end gap-3 pt-6 border-t border-slate-200 dark:border-slate-700">
                    <button type="button" onclick="document.getElementById('broadcastModal').classList.add('hidden')" class="px-6 py-3 bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-xl font-bold text-sm hover:bg-slate-200">
                        Discard
                    </button>
                    <button type="submit" class="px-6 py-3 bg-gradient-to-r from-purple-700 to-purple-600 text-white rounded-xl font-bold text-sm hover:shadow-lg transition-transform active:scale-95">
                        Execute Broadcast
                    </button>
                </div>
            </form>
...
<script>
    ...
    function viewOpportunityDetail(id) {
        ...
        if (opp) {
            ...
            `;
            if (opp.training_provider) html += `<div><p class="text-xs font-bold text-slate-500 uppercase">Training Provider</p><p class="text-sm text-slate-700 dark:text-slate-300">${opp.training_provider}</p></div>`;
            ...
            if (opp.certification) html += `<div><p class="text-xs font-bold text-slate-500 uppercase">Certification Granted</p><p class="text-sm font-bold text-blue-900 dark:text-blue-400">${opp.certification}</p></div>`;
            if (opp.description) html += `<div><p class="text-xs font-bold text-slate-500 uppercase">Program Description</p><p class="text-sm text-slate-700 dark:text-slate-300 whitespace-pre-wrap">${opp.description}</p></div>`;
            html += `
    ...
    function openBroadcastModal(opp) {
...
Please visit the Municipal KK office to apply!`;
    ...
    function toggleSpecificBroadcastRecipients() {
        ...
        document.getElementById('broadcast_specific_container').style.display = (group === 'Specific') ? 'block' : 'none';
    }
    ...
    function updateBroadcastPreview() {
        ...
        const oppTitle = opportunitiesData[document.getElementById('broadcast_opp_id').value]?.title || 'Training Program';
