...
if ($current_osy_id > 0) {
    $current_chat_user = $database->fetchOne("SELECT p.id, p.first_name, p.last_name, p.phone, p.email, p.image_path FROM osy_profiles p WHERE p.id = ?", [$current_osy_id], 'i');
}
...
<?php if ($messageResult): ?>
    <div id="msgAlertBanner"
        class="mb-4 p-3 <?php echo $messageResultType === 'error' ? 'bg-red-50 border border-red-200 text-red-800' : 'bg-green-50 border border-green-200 text-green-800'; ?> rounded-xl flex items-center gap-3">
        <span
            class="material-symbols-outlined text-lg shrink-0"><?php echo $messageResultType === 'error' ? 'error' : 'check_circle'; ?></span>
        <p class="text-sm font-medium flex-1"><?php echo $messageResult; ?></p>
        <button onclick="document.getElementById('msgAlertBanner').remove()"
            class="text-current opacity-60 hover:opacity-100"><span
                class="material-symbols-outlined text-lg">close</span></button>
    </div>
<?php endif; ?>

<div
    class="flex h-[calc(100vh-210px)] bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">

    <!-- ─── Chat List Sidebar ────────────────────────────────────── -->
    <div
        class="w-80 flex-shrink-0 border-r border-slate-200 dark:border-slate-700 flex flex-col bg-slate-50 dark:bg-slate-800/50">
        <div class="p-4 border-b border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800">
            ...
            <div class="relative">
                ...
                <input type="text" id="chatSearch" oninput="filterChats(this.value)"
                    placeholder="Search conversations..."
                    class="w-full bg-slate-100 dark:bg-slate-700 border-none rounded-xl py-2 pl-9 pr-4 text-sm focus:ring-2 focus:ring-blue-900 text-slate-900 dark:text-white">
            </div>
        </div>
...
<div class="flex-1 flex flex-col min-w-0">
    <?php if ($current_osy_id > 0 && $current_chat_user): ?>
        ...
        <!-- Chat Header -->
        <div
            class="flex-shrink-0 p-4 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between bg-white dark:bg-slate-800">
            <div class="flex items-center gap-3">
                <?php if (!empty($current_chat_user['image_path'])): ?>
                    <img src="<?php echo htmlspecialchars($current_chat_user['image_path']); ?>"
                ...
                <div>
                    <h3 class="font-bold text-slate-900 dark:text-white leading-tight">
                        <?php echo htmlspecialchars($current_chat_user['first_name'] . ' ' . $current_chat_user['last_name']); ?>
                    </h3>
                    <p class="text-xs text-slate-400 flex items-center gap-1 mt-0.5">
                        <span class="w-1.5 h-1.5 rounded-full bg-green-500 inline-block"></span>
                        <?php echo htmlspecialchars($current_chat_user['phone'] ?? 'No phone'); ?> &nbsp;|&nbsp;
                        <?php echo htmlspecialchars($current_chat_user['email'] ?? 'No email'); ?>
                    </p>
                </div>
            </div>
            <div class="flex items-center gap-2">
                <?php if (!empty($current_chat_user['phone'])): ?>
                    <a href="sms:<?php echo htmlspecialchars(preg_replace('/[^0-9+]/', '', $current_chat_user['phone'])); ?>"
                        class="px-3 py-1.5 bg-green-50 border border-green-200 text-green-700 hover:bg-green-100 rounded-lg text-xs font-bold transition-colors inline-flex items-center gap-1">
                        <span class="material-symbols-outlined text-[14px]">sms</span>SMS App
                    </a>
                    <a href="https://messages.google.com/web/conversations/new?to=<?php echo urlencode(preg_replace('/[^0-9+]/', '', $current_chat_user['phone'])); ?>"
                        target="_blank"
                        class="px-3 py-1.5 bg-blue-50 border border-blue-200 text-blue-700 hover:bg-blue-100 rounded-lg text-xs font-bold transition-colors inline-flex items-center gap-1">
                        <span class="material-symbols-outlined text-[14px]">open_in_new</span>Google Web
                    </a>
                <?php endif; ?>
                <a href="profile-detail.php?id=<?php echo $current_osy_id; ?>"
                    class="px-3 py-1.5 bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-600 rounded-lg text-xs font-bold transition-colors">View
                    Profile</a>
            </div>
        </div>
                        ...
                        <div class="max-w-[72%]">
                            ...
                            <div class="flex items-center gap-1.5 mt-1 px-1 <?php echo $isAdmin ? 'justify-end' : ''; ?>">
                                <?php if ($isAdmin && (!empty($msg['sms_status']) && $msg['sms_status'] !== 'none')): ?>
                                    <span
                                        class="material-symbols-outlined text-[11px] <?php echo $msg['sms_status'] === 'success' ? 'text-green-500' : 'text-red-400'; ?>"
                                ...
                                <?php if ($isAdmin && (!empty($msg['email_status']) && $msg['email_status'] !== 'none')): ?>
                                    <span
                                        class="material-symbols-outlined text-[11px] <?php echo $msg['email_status'] === 'success' ? 'text-green-500' : 'text-red-400'; ?>"
                                        title="Email: <?php echo $msg['email_status'] === 'success' ? 'Sent' : htmlspecialchars($msg['email_error'] ?? 'Failed'); ?>">mail</span>
                                <?php endif; ?>
                                <span
                                    class="text-[10px] text-slate-400"><?php echo date('h:i A', strtotime($msg['created_at'])); ?></span>
                                <?php if ($isAdmin): ?>
                                    <span
                                        class="material-symbols-outlined text-[11px] text-slate-400"><?php echo $msg['is_read'] ? 'done_all' : 'check'; ?></span>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
                ...
                <div class="px-4 pt-2 flex gap-2 flex-wrap">
                    <?php if (!$smsConfigured): ?>
                        <a href="settings.php?tab=notifications"
                            class="inline-flex items-center gap-1 text-[10px] font-semibold text-amber-700 bg-amber-50 border border-amber-200 px-2 py-1 rounded-full hover:bg-amber-100 transition-colors">
                            <span class="material-symbols-outlined text-[11px]">warning</span>SMS not configured – <u>Set up in
                                Settings</u>
                        </a>
                    <?php endif; ?>
                    <?php if (!$emailConfigured): ?>
                        <a href="settings.php?tab=notifications"
                            class="inline-flex items-center gap-1 text-[10px] font-semibold text-amber-700 bg-amber-50 border border-amber-200 px-2 py-1 rounded-full hover:bg-amber-100 transition-colors">
                            <span class="material-symbols-outlined text-[11px]">warning</span>Email not configured – <u>Set up in
                                Settings</u>
                        </a>
                    <?php endif; ?>
                </div>
                        ...
                        <div class="flex items-center gap-4 mt-2 px-1">
                            <label class="flex items-center gap-1.5 cursor-pointer select-none">
                                <input type="checkbox" id="chkSms"
                                    class="rounded border-slate-300 text-blue-900 focus:ring-blue-900 w-3.5 h-3.5"
                                    <?php echo !$smsConfigured ? 'disabled title="Configure Traccar SMS in Settings"' : ''; ?>>
                                <span
                                    class="text-[11px] font-semibold text-slate-500 flex items-center gap-1 <?php echo !$smsConfigured ? 'opacity-40' : ''; ?>">
                            ...
                            <label class="flex items-center gap-1.5 cursor-pointer select-none">
                                <input type="checkbox" id="chkEmail"
                                    class="rounded border-slate-300 text-blue-900 focus:ring-blue-900 w-3.5 h-3.5"
                                    <?php echo !$emailConfigured ? 'disabled title="Configure Brevo API in Settings"' : ''; ?>>
                                <span
                                    class="text-[11px] font-semibold text-slate-500 flex items-center gap-1 <?php echo !$emailConfigured ? 'opacity-40' : ''; ?>">
        ...
        <script>
            ...
            function showMsgToast(msg, type) {
                ...
                toast.className =
                    'fixed bottom-6 right-6 z-50 px-5 py-3 rounded-xl shadow-xl text-sm font-semibold flex items-center gap-2 transition-all ' +
                    (type === 'error' ? 'bg-red-600 text-white' : 'bg-green-600 text-white');
                toast.innerHTML = '<span class="material-symbols-outlined text-base">' + (type === 'error' ? 'error' :
    ...
    <?php else: ?>
        ...
        <div class="flex-1 flex flex-col items-center justify-center text-slate-400 bg-slate-50/40 dark:bg-slate-800/20">
            <div
                class="w-24 h-24 bg-white dark:bg-slate-700 rounded-full flex items-center justify-center shadow-sm mb-6 border border-slate-100 dark:border-slate-600">
            ...
            <p class="text-sm mt-2 max-w-xs text-center leading-relaxed text-slate-500">Choose a youth profile from the list
                on the left to view messages or start a new conversation.</p>
        </div>
    <?php endif; ?>
</div>
</div>
            ...
            fetch(`../api/get_messages_data.php?view=chat_list`)
                ...
                .then(res => {
                    ...
                    if (!res.success || !res.chats || res.chats.length === 0) {
                        container.innerHTML =
                            `<div class="p-8 text-center text-slate-400"><span class="material-symbols-outlined text-4xl block mb-2 opacity-30">chat_bubble_outline</span><p class="text-sm">No profiles found.</p></div>`;
                        return;
                    }

                    container.innerHTML = res.chats.map(chat => {
                        const searchName = `${chat.first_name} ${chat.last_name}`.toLowerCase();
                        const activeClass = currentChatId == chat.id ?
                            'bg-blue-50 dark:bg-blue-900/20 border-l-4 border-blue-900' : '';

                        let imageHtml = '';
                        if (chat.image_path) {
                            imageHtml =
                                `<img src="${escapeHtml(chat.image_path)}" class="w-10 h-10 rounded-full object-cover">`;
                        } else {
                            const initials = (chat.first_name.charAt(0) + chat.last_name.charAt(0))
                                .toUpperCase();
                            imageHtml =
                                `<div class="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white font-bold text-sm">${initials}</div>`;
                        }

                        let unreadHtml = '';
                        if (chat.unread_count && chat.unread_count > 0) {
                            unreadHtml =
                                `<span class="absolute -top-1 -right-1 w-4 h-4 bg-red-500 rounded-full text-[9px] flex items-center justify-center text-white font-bold border-2 border-white dark:border-slate-800">${chat.unread_count}</span>`;
                        }
                        ...
                        if (chat.last_message_time) {
                            ...
                            timeHtml =
                                `<span class="text-[10px] text-slate-400 whitespace-nowrap ml-1">${dt.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span>`;
                        }
