...
function canManageMatchingOpportunity(?array $opportunity): bool
{
    if (!$opportunity) {
        return false;
    }
...
<div>
    <nav class="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-400 font-medium mb-2">
        <a href="dashboard.php" class="hover:text-blue-900 transition-colors">Panaon KK</a>
...
<section class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-stretch mb-10">
    <div
        class="lg:col-span-2 rounded-xl bg-white dark:bg-slate-800 p-8 border border-slate-200 dark:border-slate-700 shadow-sm relative overflow-hidden">
        <div class="absolute top-0 right-0 p-4 opacity-10">
            <span class="material-symbols-outlined text-8xl">verified</span>
        </div>
        <div class="relative z-10">
            <span
                class="inline-flex items-center gap-1.5 px-3 py-1 bg-blue-100 dark:bg-blue-900/30 text-blue-900 dark:text-blue-400 font-bold text-[10px] tracking-widest uppercase rounded-full mb-4">
                Active Opportunity
            </span>
            <h1 id="heroTitle" class="text-3xl font-extrabold tracking-tight text-blue-900 dark:text-white mb-2">
                <?php echo htmlspecialchars($selectedOpportunity['title'] ?? 'No active opportunity selected'); ?></h1>
            <p id="heroDesc" class="text-slate-600 dark:text-slate-300 max-w-xl mb-6">
                <?php echo htmlspecialchars($selectedOpportunity['description'] ?? 'Select a live opportunity from the panel to see matched OSY candidates and score breakdowns.'); ?>
            </p>
            <div class="flex flex-wrap gap-3">
                <div
                    class="flex items-center gap-2 px-4 py-2 bg-slate-100 dark:bg-slate-700 rounded-lg border border-slate-200 dark:border-slate-600">
                    <span class="material-symbols-outlined text-blue-900 dark:text-blue-400 text-xl">location_on</span>
                    <span id="heroLocation"
                        class="font-semibold text-sm text-slate-900 dark:text-white"><?php echo htmlspecialchars($selectedOpportunity['location'] ?? 'No location'); ?></span>
                </div>
                <div
                    class="flex items-center gap-2 px-4 py-2 bg-slate-100 dark:bg-slate-700 rounded-lg border border-slate-200 dark:border-slate-600">
                    <span class="material-symbols-outlined text-blue-900 dark:text-blue-400 text-xl">schedule</span>
                    <span id="heroDeadline"
                        class="font-semibold text-sm text-slate-900 dark:text-white"><?php echo !empty($selectedOpportunity['deadline']) ? date('M d, Y', strtotime($selectedOpportunity['deadline'])) : 'No deadline'; ?></span>
                </div>
                <div
                    class="flex items-center gap-2 px-4 py-2 bg-slate-100 dark:bg-slate-700 rounded-lg border border-slate-200 dark:border-slate-600">
                    <span class="material-symbols-outlined text-blue-900 dark:text-blue-400 text-xl">people</span>
                    <span id="heroMatchCount" class="font-semibold text-sm text-slate-900 dark:text-white"><span
                            id="totalMatchesCount">0</span> candidates found</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="rounded-xl bg-blue-900 text-white p-8 shadow-xl flex flex-col justify-between">
        <div>
            ...
            <div>
                ...
                <select id="oppSelect" onchange="handleOpportunityChange(this.value)"
            ...
            <div class="space-y-3">
                ...
                <input type="range" name="min_score" min="0" max="100" value="<?php echo $minScore; ?>"
...
<div id="broadcastModal" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50 flex items-center justify-center p-4">
    <div class="bg-white dark:bg-slate-800 rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
        <div class="p-6 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/80 rounded-t-2xl">
            <h3 class="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2"><span
                    class="material-symbols-outlined text-blue-600">campaign</span> Configure Broadcast</h3>
            <p class="text-sm text-slate-500 mt-1">Send alerts specifically to candidates you have <span
                    class="font-bold text-green-600">Accepted/Shortlisted</span> for <span
                    id="broadcastOppName"><?php echo htmlspecialchars($selectedOpportunity['title'] ?? ''); ?></span>.
            </p>
        </div>
        <form method="POST" class="p-6 space-y-6">
            <input type="hidden" name="opportunity_id" id="broadcastOppId"
                value="<?php echo $selectedOpportunityId ?? ''; ?>">
            <input type="hidden" name="form_nonce" value="<?php echo htmlspecialchars(getFormNonce()); ?>">

            <div class="space-y-4">
                <div>
                    <label class="text-sm font-bold text-slate-700 dark:text-slate-300 mb-2 block">Choose a Template
                        (Optional)</label>
                ...
                <div>
                    ...
                    <textarea id="customMessageArea" name="custom_message" rows="5" required
                        class="w-full bg-slate-100 dark:bg-slate-700 border-none rounded-xl py-3 px-4 text-sm focus:ring-2 focus:ring-blue-900 text-slate-900 dark:text-white font-mono"
                        onkeyup="updateLivePreview()">You have been shortlisted for the opportunity "{{opportunity}}". Please prepare for next steps.</textarea>
                </div>
            </div>

            <!-- Live Preview Area -->
            <div class="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-xl border border-blue-100 dark:border-blue-800">
                <p class="text-xs font-bold text-blue-800 dark:text-blue-400 mb-2 uppercase tracking-wide">Live Preview
                    Example</p>
                ...
                <div class="flex gap-6">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="send_sms"
                            class="w-4 h-4 text-blue-900 bg-slate-100 border-slate-300 rounded focus:ring-blue-900"
                            checked>
                        <span class="text-sm text-slate-600 dark:text-slate-300 font-medium">Auto-send via Traccar
                            SMS</span>
                    </label>
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="send_email"
                            class="w-4 h-4 text-blue-900 bg-slate-100 border-slate-300 rounded focus:ring-blue-900"
                            checked>
                        <span class="text-sm text-slate-600 dark:text-slate-300 font-medium">Auto-send via SMTP
                            Email</span>
                    </label>
                </div>
            </div>
            ...
            <div class="flex justify-end gap-3 pt-4 border-t border-slate-200 dark:border-slate-700 mt-6">
                ...
                <button type="submit" name="broadcast_matches" value="1"
                    class="px-6 py-3 bg-blue-600 text-white rounded-xl font-bold text-sm hover:bg-blue-700 shadow-md">
...
<section class="mb-10">
    <div class="flex items-center justify-between border-b border-slate-200 dark:border-slate-700 pb-4 mb-6">
        <div class="flex items-center gap-3">
            <div
                class="w-10 h-10 rounded-full bg-green-100 dark:bg-green-900/30 flex items-center justify-center text-green-700 dark:text-green-400 shadow-sm">
                <span class="material-symbols-outlined">work</span>
            </div>
            <div>
                <h2 class="text-2xl font-bold tracking-tight text-slate-900 dark:text-white">Employment Pipeline</h2>
                <p class="text-sm text-slate-600 dark:text-slate-400 font-medium">Real-time candidate sorting and
                    shortlisting.</p>
            </div>
        </div>
        <span class="text-sm font-bold text-slate-500 bg-slate-100 py-1 px-3 rounded-full"><span
                id="totalMatchesCount">0</span> total matches</span>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div
            class="bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-4 border border-slate-200 dark:border-slate-700 h-[800px] flex flex-col relative">
            <div class="flex border-b border-slate-200 dark:border-slate-700 mb-4">
                <button onclick="switchTab('shortlisted')" id="tabShortlisted"
                    class="flex-1 py-2 text-sm font-bold border-b-2 border-blue-600 text-blue-600">Shortlisted (<span
                        id="pendingCount">0</span>)</button>
                <button onclick="switchTab('rejected')" id="tabRejected"
                    class="flex-1 py-2 text-sm font-bold border-b-2 border-transparent text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-300">Rejected
                    (<span id="rejectedCount">0</span>)</button>
            </div>
        ...
        <div
            class="bg-blue-50/50 dark:bg-blue-900/10 rounded-2xl p-4 border border-blue-100 dark:border-blue-800/50 h-[800px] flex flex-col">
            <div class="flex justify-between items-center mb-4 px-2">
                ...
                <span id="acceptedCount"
                    class="text-xs font-bold text-blue-700 bg-blue-100 dark:bg-blue-900 px-2 py-1 rounded-full">0</span>
            </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto shadow-2xl">
        ...
        <div class="p-6 space-y-4">
            <div class="grid grid-cols-2 gap-4 text-sm">
                <div><span class="block text-slate-500 text-xs">Name</span> <span id="mdlName"
                        class="font-bold text-slate-900 dark:text-white"></span></div>
                <div><span class="block text-slate-500 text-xs">Age</span> <span id="mdlAge"
                        class="font-bold text-slate-900 dark:text-white"></span></div>
                <div><span class="block text-slate-500 text-xs">Gender</span> <span id="mdlGender"
                        class="font-bold text-slate-900 dark:text-white"></span></div>
                <div><span class="block text-slate-500 text-xs">Barangay</span> <span id="mdlBarangay"
                        class="font-bold text-slate-900 dark:text-white"></span></div>
                <div class="col-span-2"><span class="block text-slate-500 text-xs">Educational Attainment</span> <span
                        id="mdlEdu" class="font-bold text-slate-900 dark:text-white"></span></div>
                <div class="col-span-2"><span class="block text-slate-500 text-xs">Primary Skill</span> <span
                        id="mdlPrimarySkill" class="font-bold text-slate-900 dark:text-white"></span></div>
                <div class="col-span-2"><span class="block text-slate-500 text-xs">Other Skills</span> <span
                        id="mdlSkills" class="font-bold text-slate-900 dark:text-white"></span></div>
                <div class="col-span-2"><span class="block text-slate-500 text-xs">Interests</span> <span
                        id="mdlInterests" class="font-bold text-slate-900 dark:text-white"></span></div>
            </div>
        </div>
    </div>
</div>
