...
<div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
    <div class="space-y-1">
        ...
        <p class="text-slate-600 max-w-2xl">Browse and apply to job openings, training programs, and scholarships
            matching your profile.</p>
        <?php else: ?>
        <h1 class="text-4xl font-extrabold text-blue-900 tracking-tight">Opportunity Management</h1>
        <p class="text-slate-600 max-w-2xl">Curate and manage vocational training, employment roles, and scholarships.
        </p>
        <?php endif; ?>
    </div>
    <?php if ($canCreate): ?>
    <div class="flex flex-col sm:flex-row gap-3">
        <button onclick="openCreateModal('Job Opening')"
            class="px-6 py-3 bg-gradient-to-r from-blue-900 to-blue-800 text-white rounded-xl font-bold text-sm shadow-lg hover:shadow-xl transition-all flex items-center gap-2">
    ...
    <form id="filterForm" method="GET" class="grid grid-cols-1 lg:grid-cols-3 gap-4 items-end">
        <div>
            ...
            <input type="text" id="filter_search" name="search"
                value="<?php echo htmlspecialchars($filters['search']); ?>" placeholder="Search opportunities..."
        ...
        <div>
            ...
            <select id="filter_type" name="type"
                ...
                <option value="Job Opening" <?php echo $filters['type'] === 'Job Opening' ? ' selected' : ''; ?>>Job
                    Opening</option>
                <option value="Vocational Training"
                    <?php echo $filters['type'] === 'Vocational Training' ? ' selected' : ''; ?>>Training Opportunity
                </option>
            </select>
        </div>
        ...
        <div class="flex gap-3">
            <button type="button" onclick="resetFilters()"
                class="w-full py-3 bg-slate-100 dark:bg-slate-700 text-slate-900 dark:text-white rounded-xl font-bold text-sm text-center hover:bg-slate-200 dark:hover:bg-slate-600 transition-all">Reset</button>
        </div>
    </form>
</div>
...
<div id="opportunitiesContainer">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <?php for ($i = 0; $i < 4; $i++): ?>
        <div
            class="bg-white dark:bg-slate-800 rounded-xl overflow-hidden shadow-sm border border-slate-200 dark:border-slate-700">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <form id="opportunityForm" method="POST" class="p-6 space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="compensation" id="opp_compensation"
                ...
                <div>
                    ...
                    <textarea name="benefits" id="opp_benefits" rows="2"
                        placeholder="e.g., Health insurance, 13th month pay"
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="text-sm font-bold text-slate-700 dark:text-slate-300 mb-2 block">Minimum
                            Age</label>
                        <input type="number" name="age_min" id="opp_age_min" min="1"
                            class="w-full bg-slate-100 dark:bg-slate-700 border-none rounded-xl py-3 px-4 text-sm focus:ring-2 focus:ring-blue-900 text-slate-900 dark:text-white">
                    </div>
                    <div>
                        <label class="text-sm font-bold text-slate-700 dark:text-slate-300 mb-2 block">Maximum
                            Age</label>
                        <input type="number" name="age_max" id="opp_age_max" min="1"
                            class="w-full bg-slate-100 dark:bg-slate-700 border-none rounded-xl py-3 px-4 text-sm focus:ring-2 focus:ring-blue-900 text-slate-900 dark:text-white">
                    </div>
                </div>
...
<div id="detailModal" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50">
    <div class="flex items-center justify-center min-h-screen p-4">
        <div
            class="bg-white dark:bg-slate-800 rounded-2xl max-w-lg w-full overflow-hidden shadow-2xl transform transition-all">
            <div
                class="px-6 py-4 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between bg-slate-50 dark:bg-slate-700/50">
            ...
            <div
                class="px-6 py-4 border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-700/50 flex justify-end">
                <button onclick="document.getElementById('detailModal').classList.add('hidden')"
                    class="px-4 py-2 bg-slate-200 dark:bg-slate-600 text-slate-800 dark:text-slate-200 rounded-lg text-sm font-semibold hover:bg-slate-300 dark:hover:bg-slate-500 transition-colors">Close</button>
            </div>
        </div>
    </div>
</div>
    ...
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="bg-white dark:bg-slate-800 rounded-2xl max-w-md w-full">
            <div class="p-6">
                ...
                <p class="text-slate-600 dark:text-slate-300 text-center text-sm mb-6">Are you sure? This action cannot
                    be undone.</p>
...
function viewOpportunityDetail(id) {
    ...
    if (opp) {
        ...
        if (opp.compensation || opp.benefits || opp.certification) {
            ...
            if (opp.compensation) html +=
                `<div><p class="text-xs font-semibold text-slate-500 uppercase">Compensation</p><p class="text-sm font-medium text-slate-900 dark:text-white">${opp.compensation}</p></div>`;
            if (opp.benefits) html +=
                `<div><p class="text-xs font-semibold text-slate-500 uppercase">Benefits</p><p class="text-sm text-slate-700 dark:text-slate-300">${opp.benefits}</p></div>`;
            if (opp.certification) html +=
                `<div><p class="text-xs font-semibold text-slate-500 uppercase">Certification Required</p><p class="text-sm text-slate-700 dark:text-slate-300">${opp.certification}</p></div>`;
            html += `</div>`;
        }
        html += `
...
function showToast(message, type = 'success') {
    ...
    toast.className =
        `toast fixed right-4 bottom-4 z-50 max-w-sm w-full px-4 py-3 rounded-2xl shadow-xl text-sm font-semibold ${type === 'success' ? 'bg-emerald-600 text-white' : 'bg-rose-600 text-white'}`;
...
var sessionUserId = "<?php echo $_SESSION['user_id']; ?>";
var isProvider = <?php echo $isProvider ? 'true' : 'false'; ?>;
...
function renderOppCard(opp) {
    ...
    if (sessionRole === 'youth' && opp.match_score !== null && opp.type === 'Job Opening') {
        ...
        const scoreColor = score >= 70 ? 'bg-emerald-100 text-emerald-800' : (score >= 40 ?
            'bg-amber-100 text-amber-800' : 'bg-slate-100 text-slate-600');
        scoreBadge =
            `<div class="mb-3"><span class="inline-flex items-center px-2 py-1 rounded text-xs font-bold ${scoreColor}"><span class="material-symbols-outlined text-[14px] mr-1">auto_awesome</span>${score}% Match</span></div>`;
    }
    ...
    let desc = opp.description ?
        `<p class="text-sm text-slate-600 dark:text-slate-300 line-clamp-2">${opp.description}</p>` : '';
    let comp = opp.compensation ?
        `<div><p class="text-xs font-semibold text-slate-500 uppercase">Compensation</p><p class="text-lg font-bold text-slate-900 dark:text-white">${opp.compensation}</p></div>` :
        '';
    let cert = opp.certification ?
        `<div><p class="text-xs font-semibold text-slate-500 uppercase">Certification</p><p class="text-sm text-slate-700 dark:text-slate-300">${opp.certification}</p></div>` :
        '';

    const deadline = opp.deadline ? new Date(opp.deadline).toLocaleDateString('en-US', {
        month: 'short',
        day: '2-digit',
        year: 'numeric'
    }) : 'No deadline';
    ...
    let buttons =
        `<button onclick="viewOpportunityDetail(${opp.id})" class="flex-1 py-2 px-3 bg-blue-900 text-white rounded-lg text-sm font-semibold hover:bg-blue-800 transition-colors flex items-center justify-center gap-2"><span class="material-symbols-outlined text-base">visibility</span> View Details</button>`;

    if (isProvider && parseInt(opp.provider_id) === parseInt(sessionUserId)) {
        buttons +=
            `<button onclick="openEditModal(${opp.id})" class="py-2 px-3 hover:bg-slate-200 dark:hover:bg-slate-600 rounded-lg text-slate-600 dark:text-slate-300 transition-colors"><span class="material-symbols-outlined">edit</span></button>
        ...
        if (!opp.application_status && opp.status === 'Open') {
            buttons +=
                `<button id="apply-btn-${opp.id}" onclick="applyToOpportunity(${opp.id})" class="py-2 px-3 bg-green-600 text-white rounded-lg text-sm font-semibold hover:bg-green-700 transition-colors flex items-center gap-2"><span class="material-symbols-outlined text-base">send</span> Apply</button>`;
        } else if (opp.application_status === 'Pending') {
            buttons +=
                `<button id="cancel-btn-${opp.id}" onclick="cancelApplication(${opp.id})" class="py-2 px-3 bg-red-600 text-white rounded-lg text-sm font-semibold hover:bg-red-700 transition-colors flex items-center gap-2"><span class="material-symbols-outlined text-base">cancel</span> Cancel Application</button>`;
        } else if (opp.application_status) {
            buttons +=
                `<div class="py-2 px-3 bg-slate-200 dark:bg-slate-600 text-slate-600 dark:text-slate-300 rounded-lg text-sm font-semibold flex items-center gap-2 cursor-not-allowed"><span class="material-symbols-outlined text-base">check_circle</span> Applied (${opp.application_status})</div>`;
        } else {
            buttons +=
                `<div class="py-2 px-3 bg-slate-200 dark:bg-slate-600 text-slate-500 dark:text-slate-400 rounded-lg text-sm font-semibold flex items-center gap-2 cursor-not-allowed"><span class="material-symbols-outlined text-base">block</span> Closed</div>`;
        }
    } else if (sessionRole === 'lydo' || sessionRole === 'sk_chairman') {
        buttons +=
            `<button onclick="viewApplications(${opp.id})" class="py-2 px-3 bg-purple-600 text-white rounded-lg text-sm font-semibold hover:bg-purple-700 transition-colors flex items-center gap-2"><span class="material-symbols-outlined text-base">people</span> View Applications</button>`;
    }
    ...
    fetch(`../api/get_opportunities_data.php?${params}`)
        ...
        .then(res => {
            ...
            if (!res.success || !res.opportunities.length) {
                container.innerHTML =
                    `<div class="text-center py-16 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700"><span class="material-symbols-outlined text-5xl text-slate-300 mb-3">work_off</span><p class="text-slate-500 font-semibold text-lg">No opportunities found</p></div>`;
                return;
            }
            ...
            if (type === 'All' && res.opportunities.length > 0) {
                ...
                const otherOpps = res.opportunities.filter(o => o.type !== 'Vocational Training' && o.type !==
                    'Job Opening');

                let html = `<div class="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">`;
                // Left: Training + Other
                html +=
                    `<div><h2 class="text-base font-bold text-purple-700 dark:text-purple-400 mb-4 flex items-center gap-2 uppercase tracking-wide"><span class="material-symbols-outlined text-xl">school</span> Training Opportunities</h2>`;
                if (trainingOpps.length) {
                    html += `<div class="space-y-6">${trainingOpps.map(renderOppCard).join('')}</div>`;
                } else {
                    html +=
                        `<div class="text-center py-10 bg-white dark:bg-slate-800 rounded-xl border border-slate-200"><span class="material-symbols-outlined text-4xl text-slate-300">school</span><p class="text-slate-400 text-sm mt-2">None available.</p></div>`;
                }
                if (otherOpps.length) {
                    html +=
                        `<h2 class="text-base font-bold text-yellow-700 dark:text-yellow-400 mt-8 mb-4 flex items-center gap-2 uppercase tracking-wide"><span class="material-symbols-outlined text-xl">workspace_premium</span> Scholarships</h2>`;
                ...
                // Right: Jobs
                html +=
                    `<div><h2 class="text-base font-bold text-blue-900 dark:text-blue-400 mb-4 flex items-center gap-2 uppercase tracking-wide"><span class="material-symbols-outlined text-xl">work</span> Job Openings</h2>`;
                if (jobOpps.length) {
                    html += `<div class="space-y-6">${jobOpps.map(renderOppCard).join('')}</div>`;
                } else {
                    html +=
                        `<div class="text-center py-10 bg-white dark:bg-slate-800 rounded-xl border border-slate-200"><span class="material-symbols-outlined text-4xl text-slate-300">work_off</span><p class="text-slate-400 text-sm mt-2">None available.</p></div>`;
                }
                html += `</div></div>`;
                ...
                // Single grid
                container.innerHTML =
                    `<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">${res.opportunities.map(renderOppCard).join('')}</div>`;
            }
        })
        .catch(() => {
            loader.classList.add('hidden');
...
if (searchEl) {
    ...
    searchEl.addEventListener('input', () => {
        ...
        debounce = setTimeout(loadOpportunities, 400);
    });
}
